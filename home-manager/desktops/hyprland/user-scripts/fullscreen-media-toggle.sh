...
restore_window_state() {
	...
	IFS='|' read -r cur_float _cur_pin <<<"$(hyprctl clients -j | jq -r ".[]|select(.address==\"${addr}\")|
\"\(.floating)|\(.pinned)\"")"
...
main() {
	...
	# Variables addr, fs, pin are assigned via eval from find_media_window
	eval "$(find_media_window)"
	# shellcheck disable=SC2154  # addr, fs, pin assigned via eval
	[ -z "$addr" ] && exit 0

	# shellcheck disable=SC2154  # fs, pin, addr assigned via eval
	if [ "$fs" != "0" ]; then
		# Exiting fullscreen
		restore_window_state
	elif [ "$pin" = "true" ]; then
