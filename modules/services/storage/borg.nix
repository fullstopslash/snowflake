...
in
{
  options.myModules.services.storage.borg = {
    ...
    borgPort = lib.mkOption {
      ...
      default = "${builtins.toString config.identity.networking.ports.tcp.ssh}";
    ...
    borgCacheDir =
      let
        persistFolder = lib.optionalString isImpermanent config.hardware.host.persistFolder;
      in
      lib.mkOption {
        type = lib.types.str;
    ...
    lib.mkMerge [
      {
        ...
        sops.secrets = {
          # Borg passphrase for encrypted backups
          "passwords/borg" = {
          ...
          # SSH key for borg server connection
          "keys/ssh/borg" = {
            sopsFile = "${sopsFolder}/shared.yaml";
            owner = "root";
            group = if pkgs.stdenv.isLinux then "root" else "wheel";
      ...
      (lib.mkIf pkgs.stdenv.isLinux {
        ...
        systemd =
          ...
          in
          {
            tmpfiles.rules =
              let
                user = config.users.users.${config.identity.primaryUsername}.name;
                group = config.users.users.${config.identity.primaryUsername}.group;
              in
